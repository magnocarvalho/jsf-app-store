(function($) {
  $.fn.extend({
    createCalendar: function(options) {
      ...
      function createC(elem, year, month) {
        var table =
            '<table class="calendar w_full t_align_c"><tr class="bg_light_2 color_grey"><th>M</th><th>T</th><th>W</th><th>T</th><th>F</th><th>S</th><th>S</th></tr><tr>',
          mon = month,
        ...
        while (d.getMonth() == mon) {
          if (mn && day == d.getDate())
            table +=
              '<td class="active"><span class="d_inline_m color_pink r_corners border_color_pink">' +
              d.getDate() +
              "</span></td>";
    ...
    accordion: function() {
      ...
      item
        .filter(".active")
        .children("dt")
        .removeClass("color_dark")
        .addClass("color_scheme");
      ...
      item.on("click", ".accordion_link", function() {
        ...
        if (self.closest(accordion).hasClass("toggle")) {
          self
            .toggleClass("color_dark color_scheme")
            .next()
            .stop()
            .slideToggle()
            .parent()
    ...
    formSelect: function() {
      return $(this).each(function() {
        ...
        for (var i = 0; i < len; i++) {
          list.append(
            '<li class="tr_delay fs_medium color_grey">' +
              select
                .children("option")
                .eq(i)
                .val() +
  ...
  $.extend({
    ...
    megaMenu: function() {
      ...
      (posLeft =
        mMenu.closest("li").offset().left -
        mMenu.closest(".container").offset().left),
    ...
    sideMenu: function() {
      ...
      sideMenu.Deactivate = function() {
        if (!oButton.hasClass("active")) return false;
        if (sideContainer.hasClass("active"))
          sideContainer.stop().animate({ right: -280 });
        oButton
          .removeClass("active")
          .stop()
          .animate({
      ...
      sideMenu.clickEvents = function() {
        ...
        sMenu.on("click", ".has_sub_menu a", function(e) {
          ...
          if (
            self.next("ul").length ||
            self.next(".mega_menu_container").length
          ) {
            self
              .parent()
              .siblings(".has_sub_menu")
              .removeClass("active")
              .children("ul")
              .slideUp();
            self
    ...
    responsiveMenu: function() {
      ...
      rMenu.Activate = function() {
        ...
        header
          .off("click")
          .on("click.responsivemenu", "#menu_button", rMenu.openClose);
        header.on(
          "click.responsivemenu",
          ".main_menu li a",
          rMenu.openCloseSubMenu
        );
        nav
          .find(".touch_open_sub")
          .removeClass("touch_open_sub")
      ...
      rMenu.Deactivate = function() {
        ...
        nav
          ...
          .removeClass("prevented");
        $("#menu_button")
          .removeClass("color_grey_light_2")
          .addClass("color_blue");
        if ($("html").hasClass("md_touch"))
          header
            .off("click")
            .on("click.touch", ".main_menu li a", rMenu.touchOpenSubMenu);
      };
    ...
    backToTop: function(offset) {
      ...
      backToTop.positionButton = function() {
        var offset =
          $(".container").offset().left + $(".container").outerWidth() + 10;
    ...
    showSearchForm: function() {
      ...
      header
        .off(".search")
        .on("click.search", ".search_buttons button", function() {
